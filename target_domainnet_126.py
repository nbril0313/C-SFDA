...
def train_target_domain(args):
    logging.info(f"Start target training on {args.data.src_domain}-{args.data.tgt_domain}...")
    ...
    if args.distributed:
        ...
        model = CustomDistributedDataParallel(model, device_ids=[args.gpu], find_unused_parameters=True, broadcast_buffers=False)
    ...
    save_path = os.path.join('/home/ncmssh0313/C-SFDA_Source-Free-Domain-Adaptation/checkpoints/', filename)
...
def train_csfda(train_loader, val_loader, model, optimizer, args):
    ...
    if args.data.dataset == "VISDA-C":
        num_class = 12
    elif args.data.dataset == "office-home":
        num_class = 65
    else:
        num_class = 126
    ...
    for epoch in range(args.learn.start_epoch, args.learn.epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            squeezed_feats_con = torch.squeeze(feats_con[ind_remove])
            if squeezed_feats_con.nelement() == 0:
                print("No features to normalize.")
                continue  # Skip this iteration or handle it differently if in a loop

            # Check if there are not enough dimensions or if the second dimension is not valid for normalization
            elif squeezed_feats_con.dim() == 1 or squeezed_feats_con.size(0) == 1:
                # If there's only one dimension and it's a single element, skip normalization
                if squeezed_feats_con.dim() == 1 and squeezed_feats_con.size(0) == 1:
                    print("Only one feature to normalize, which is trivial.")
                    f1 = squeezed_feats_con
                else:
                    # Otherwise, normalize over the only dimension available
                    f1 = F.normalize(squeezed_feats_con, dim=0)
            else:
                # Safe to normalize over the second dimension
                f1 = F.normalize(squeezed_feats_con, dim=1)
            
            # f1       = F.normalize(torch.squeeze(feats_con[ind_remove]), dim=1)
            
            squeezed_feats_k = torch.squeeze(feats_k[ind_remove])
            if squeezed_feats_k.nelement() == 0:
                print("No features to normalize.")
                continue  # Skip this iteration or handle it differently if in a loop

            # Check if there are not enough dimensions or if the second dimension is not valid for normalization
            elif squeezed_feats_k.dim() == 1 or squeezed_feats_k.size(0) == 1:
                # If there's only one dimension and it's a single element, skip normalization
                if squeezed_feats_k.dim() == 1 and squeezed_feats_k.size(0) == 1:
                    print("Only one feature to normalize, which is trivial.")
                    f2 = squeezed_feats_k
                else:
                    # Otherwise, normalize over the only dimension available
                    f2 = F.normalize(squeezed_feats_k, dim=0)
            else:
                # Safe to normalize over the second dimension
                f2 = F.normalize(squeezed_feats_k, dim=1)
            # f2       = F.normalize(torch.squeeze(feats_k[ind_remove]), dim=1)
            
            if f1.dim() == 1:
                f1 = f1.unsqueeze(0)  # Convert from [feature_dimensions] to [1, feature_dimensions]
            if f2.dim() == 1:
                f2 = f2.unsqueeze(0)
            ...
            loss_coef = loss_coef * (1- 0.001* torch.exp(-1/difficulty_score))
            con_coeff =  con_coeff * np.exp(-0.0001)
            ...
            unsupervised_losses[ind] = loss_cls_rem
            ...
            accuracies_np = [acc.cpu().numpy() for acc in accuracies if isinstance(acc, torch.Tensor)],
            ...
            # pseudo_label_acc=np.array(accuracies.cpu()),
            pseudo_label_acc=np.array(accuracies_np),
            acc_class=np.array(acc_classes),
            conf=conf_thress.cpu().numpy(),
            unc=uncertainty_thresholds.cpu().numpy(),
            labeled_loss_coeff=loss_coefs.cpu().numpy(),
            # con_coeff=np.array(con_coeffs) if isinstance(con_coeffs, list) else con_coeffs.cpu().numpy(),
            con_coeff=con_coeffs,
            ce_loss=loss_classes.cpu().numpy(),
            # con_loss=con_losses.cpu().numpy(),
            con_loss=con_losses.detach().numpy(),
            # prop_loss=unsupervised_losses.cpu().numpy(),
            prop_loss=unsupervised_losses.detach().numpy(),
            sel_Samples=np.array(sel_Samples) if isinstance(sel_Samples, list) else sel_Samples.cpu().numpy(),
            unsel_samples=np.array(unsel_samples) if isinstance(unsel_samples, list) else unsel_samples.cpu().numpy())
